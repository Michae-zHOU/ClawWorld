...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="group rounded-2xl p-6 border border-slate-300 bg-white shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
      <div className="w-full aspect-square rounded-xl mb-4 flex items-center justify-center text-7xl bg-blue-100 transition-transform duration-300 group-hover:scale-105">
        {product.emoji}
      </div>
      ...
      <div className="flex items-center justify-between mt-auto">
        <div className="text-xs font-semibold px-2.5 py-1 rounded-full bg-blue-100 text-blue-700 border border-blue-200">
          +{product.points} 💊pts
        </div>
        <button 
          onClick={handleAdd}
          className="text-white px-4 py-2 rounded-lg font-medium transition-all duration-200 hover:shadow-md active:scale-95 bg-blue-600 hover:bg-blue-700"
        >
          Add to Cart
        </button>
      </div>
